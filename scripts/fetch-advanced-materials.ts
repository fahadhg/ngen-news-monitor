...
async function main() {
  ...
  console.log(buildPerigonQuery(advancedMaterials));
  console.log();

  const supabase = getSupabaseClient();
  const result = await fetchArticlesForCluster(advancedMaterials, supabase);

  console.log(`numResults: ${result.numResults}`);
  console.log(`articles returned: ${result.articles?.length ?? 0}`);

  const { stored, error } = await storeArticles(supabase, advancedMaterials, result.articles ?? []);
  if (error) {
