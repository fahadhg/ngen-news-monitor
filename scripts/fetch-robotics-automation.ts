...
async function main() {
  ...
  console.log(buildPerigonQuery(roboticsAutomation));
  console.log();

  const supabase = getSupabaseClient();
  const result = await fetchArticlesForCluster(roboticsAutomation, supabase);

  console.log(`numResults: ${result.numResults}`);
  console.log(`articles returned: ${result.articles?.length ?? 0}`);

  const { stored, error } = await storeArticles(supabase, roboticsAutomation, result.articles ?? []);
  if (error) {
