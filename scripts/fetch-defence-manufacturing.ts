...
async function main() {
  ...
  console.log(buildPerigonQuery(defenceManufacturing));
  console.log();

  const supabase = getSupabaseClient();
  const result = await fetchArticlesForCluster(defenceManufacturing, supabase);

  console.log(`numResults: ${result.numResults}`);
  console.log(`articles returned: ${result.articles?.length ?? 0}`);

  const { stored, error } = await storeArticles(supabase, defenceManufacturing, result.articles ?? []);
  if (error) {
